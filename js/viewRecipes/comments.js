//comments.js
// este archivo maneja la carga, renderización y funcionalidad de los comentarios en una receta


// Renderiza un comentario individual con soporte para comentarios hijos
export function createCommentHtml(
  comment,
  defaultImageUrl,
  isChild = false,
  postId = null
  ...
  const viewCommentsButton = `<a href="commentThread.php?commentId=${
    comment.commentId
  }" class="author-link   me-2">
         <i class="bi bi-chat"></i>${
           comment.childCount && comment.childCount > 0
             ? ` ${comment.childCount}`
             : "0"
         }
       </a>`;
  ...
  const likeButton = `<button class="comment-like-btn ${
  ...
  const ratingHtml =
    comment.rating && Number(comment.rating) > 0
      ? `
  ...
  return `
        <div class="comment " data-comment-id="${comment.commentId}">
            <img class="author-image" src="${imageUrl}" alt="Perfil de ${
    comment.userName
  }">
             <div class="comment-body">
                 ...
                 <div class="comment-actions">
                     ${likeButton}
                     ${viewCommentsButton}
                     <button class="report-btn btn btn-sm ms-2" data-target-type="comment" data-target-id="${
                       comment.commentId
                     }" type="button" aria-label="Denunciar comentario"><i class="bi bi-flag"></i></button>
                 </div>
             </div>
        </div>
    `;
...
export function loadComments(postId, defaultImageUrl) {
  ...
  fetch(`../getComment.php?postId=${postId}`)
    ...
    .then((comments) => {
      ...
      if (!comments || comments.length === 0) {
        ...
        commentsContainer.innerHTML = comments
          .map((comment) =>
            createCommentHtml(comment, defaultImageUrl, false, postId)
          )
          .join("");
    ...
    .catch((error) => {
      ...
      commentsContainer.innerHTML =
        '<p class="error-message">No se pudieron cargar los comentarios. Inténtalo más tarde.</p>';
    });
}
...
export function setupCommentHandlers(postId, defaultImageUrl) {
  ...
  document.addEventListener("click", (e) => {
    if (
      e.target.classList.contains("comment-like-btn") ||
